...
describe("Edge Cases and Additional Coverage Tests", () => {
  ...
  const testRouter = new Router()
    ...
    .post("/router-echo", (ctx) => ({ echoed: ctx.body }));
  ...
  describe("Response Formatting", () => {
    ...
    it("should format array response correctly", async () => {
  ...
  //       "http://localhost:3000"
  //     );
  //     expect(res.headers).to.have.property("access-control-allow-credentials");
  //   });
  ...
  //     const res = await client.get("/head-test");
  ...
  //     if (res.ok) {
  //       const data: any = res.data.data || res.data;
  //       expect(data).to.have.property("echoed", "test");
  //     }
  //   });
  // });
